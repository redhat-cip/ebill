...
-author("Grégoire Lejeune <gregoire.lejeune@free.fr>").
    ...
    case filter_hints(parse(tokenise(string:to_upper(Date), []), Now, Opts)) of
        ...
        Date1 = {{Y, M, D}, {H, M1, S}}
          when is_number(Y), is_number(M),
            ...
            {D2, T2} = update_date(Date1),
            case calendar:valid_date(D2) of
                true -> {D2, T2};
...
%{D2, T2} = update_date({Year, Month, Day}, {Hour, Minute, Sec})
update_date({{Year, Month, Day}, {Hour, Minute, Sec}}) ->
  if 
    Hour > 24 -> 
      Hour1 = Hour - 24,
      {Year1, Month1, Day1} = calendar:gregorian_days_to_date(calendar:date_to_gregorian_days({Year, Month, Day}) + 1),
      {{Year1, Month1, Day1}, {Hour1, Minute, Sec}};
    Hour < 0 -> 
      Hour1 = 24 + Hour,
      {Year1, Month1, Day1} = calendar:gregorian_days_to_date(calendar:date_to_gregorian_days({Year, Month, Day}) - 1),
      {{Year1, Month1, Day1}, {Hour1, Minute, Sec}};
    true -> {{Year, Month, Day}, {Hour, Minute, Sec}}
  end.
